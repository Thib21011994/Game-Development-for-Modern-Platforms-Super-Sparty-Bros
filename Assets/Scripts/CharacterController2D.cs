...
public class CharacterController2D : MonoBehaviour {
	...
	void Update()
	{
        ...
        else if (_canDoubleJump && CrossPlatformInputManager.GetButtonDown("Jump")) // if can double jump AND jump button pressed, then allow the player to double jump
        {
            DoJump();
    ...
    public void CheckIfUnderWater()
    {
        if (playerIsUnderWater)
        {
            _isGrounded = false;
            _canDoubleJump = true;
            _animator.SetBool("Swimming", playerIsUnderWater);
